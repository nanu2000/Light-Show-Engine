...
private:
	...
	****************************/
	...
	const unsigned
	int				AUDIO_RATE		= 22050;			//Audio Rate.(frequency) 22050 for LQ sound (faster) 44100 for HQ sound

	const unsigned 
	int				AUDIO_CHANNEL	= 1;				//Audio Channel	2 for sterio, 1 for mono

	const unsigned
	int				AUDIO_BUFFER	= 256;				//Audio Bufffer (chunk size) Bytes used per output sample
