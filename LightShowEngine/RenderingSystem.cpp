...
void RenderingSystem::render(PointLightShadowMap & pointLightDepthMap, DirectionalLightShadowMap & directionalLightDepthMap)
{
	/******************************************|
	...
	if (currentCamera)
	{
		...
		for (unsigned int i = 0; i < animatedModels.size(); i++)
		{
			...
			{
				addModelToProperRenderVector(*animatedModels[i], entity, transparentModels, nonTransparentModels);
			}
		}
		...
		for (unsigned int i = 0; i < models.size(); i++)
		{
			...
			if (currentScene->isEntityActive(entity))
			{
				addModelToProperRenderVector(*models[i], entity, transparentModels, nonTransparentModels);
			}
		}
		...
		for (unsigned int i = 0; i < nonTransparentModels.size(); i++)
		{
			...
			{
				supplyLitShaderUniforms(*model.shader, pointLightDepthMap, directionalLightDepthMap, *currentCamera);
			}
			else
			{
				supplyDefaultShaderUniforms(*model.shader, *currentCamera);
			}
		...
		if (transparentModels.empty() == false)
		{
			...
			sort(transparentModels.begin(), transparentModels.end(),
				[&](const RenderModel & a, const RenderModel & b) -> bool
			{
				return glm::distance(a.model->transform.position, currentCamera->position) > glm::distance(b.model->transform.position, currentCamera->position);
			});
	...
	if (ShaderBase::getShaderTask() == SHADER_TASK::Normal_Render_Task)
	{
		...
		{
			renderParticles(*particles[i], *currentCamera);
		}
...
void RenderingSystem::renderParticles(Particles & particles, Camera & currentCamera)
{
	if (DefaultShader * thisShader = currentScene->getComponent<DefaultShader>(particles.getEntityID()))
	{
		...
		{
			supplyParticleShaderUniforms(*thisShader, currentCamera);
...
ShaderBase * RenderingSystem::prepareShader(const int32_t & entity, PointLightShadowMap & pointLightDepthMap, DirectionalLightShadowMap & directionalLightDepthMap, Camera & currentCamera)
{
	...
	if (thisShader = currentScene->getComponent<DefaultShader>(entity))
	{
		if (thisShader->getShaderType() == SHADER_TYPE::Default)
		{
			if (currentScene->isEntityActive(entity))
			{
				...
				supplyDefaultShaderUniforms(*thisShader, currentCamera);
				return thisShader;
			}
		}
	}
	...
	if (thisShader = currentScene->getComponent<LitShader>(entity))
	{
		if (thisShader->getShaderType() == SHADER_TYPE::Default)
		{
			...
			{
				thisShader->useProgram();
				supplyLitShaderUniforms(*thisShader, pointLightDepthMap, directionalLightDepthMap, currentCamera);
				return thisShader;
			}
		}
	}
...
void RenderingSystem::supplyLitShaderUniforms(ShaderBase & shader, PointLightShadowMap & pointLightDepthMap, DirectionalLightShadowMap & directionalLightDepthMap, Camera & currentCamera)
{
	supplyDefaultShaderUniforms(shader, currentCamera);
	
	glActiveTexture(GL_TEXTURE0 + Shaders::DEPTH_MAP_LOCATION_OMNIDIRECTIONAL);
	glUniform1i
	(
		Shaders::getUniformLocation(shader.getProgramID(), Shaders::UniformName::PointShadowMap),
	...
	if (pointLightDepthMap.isActive())
	{
		glUniform1f
		(
			Shaders::getUniformLocation(shader.getProgramID(), Shaders::UniformName::FarPlane),
			pointLightDepthMap.getFarPlane()
		);

		glUniform3fv
		(
			Shaders::getUniformLocation(shader.getProgramID(), Shaders::UniformName::LightPosition),
			1,
	...
	glUniformMatrix4fv
	(
		Shaders::getUniformLocation(shader.getProgramID(), Shaders::UniformName::LightSpaceMatrix),
		1,
		GL_FALSE,
	...
	glUniform1i
	(
		Shaders::getUniformLocation(shader.getProgramID(), Shaders::UniformName::DirectionalShadowMap),
...
void RenderingSystem::supplyDefaultShaderUniforms(ShaderBase & shader, Camera & currentCamera)
{
	...
	glUniformMatrix4fv
	(
		Shaders::getUniformLocation(programID, Shaders::UniformName::ProjectionMatrix),
		1,
		GL_FALSE,
...
void RenderingSystem::supplyParticleShaderUniforms(ShaderBase & particleShader, Camera & currentCamera)
{
...
void RenderingSystem::addModelToProperRenderVector(ModelBase& model, const int32_t& entity, std::vector<RenderModel>& transparent, std::vector<RenderModel>& notTransparent)
{
	ShaderBase* thisShader;
