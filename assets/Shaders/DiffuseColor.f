...
float directionalShadowCalculation(vec4 fragPosLightSpace) {
    ...
    for (int i = 0; i < SHADOW_FILTERING; ++i) {
        float pcfDepth = texture(directionalShadowMap, projCoords.xy + sampleOffsetDirections[i].xy * filterAmount).r;
...
vec3 CalcDirLight(DirectionalLight light, vec3 normal, vec3 viewDir) {
    ...
    return (ambient + (1.0f - directionalShadowCalculation(fragmentPositionLightSpace_o)) * (diffuse));
}
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, float shadowCalculation) {
    ...
    ambient *= attenuation;
    diffuse *= attenuation;
    return (ambient + shadowCalculation * (diffuse));
}
